...
def load_irish_data():
    ...
    for line in readlines:
        ...
        #audio = load_wave(wav_file)[0]
        #if len(wav_text) > text_max_length or len(audio) > audio_max_sample_length:
        #    continue
...
irish_data = load_irish_data()

train_num = int(len(irish_data) * TRAIN_RATE)

train_list, eval_list = irish_data[:train_num], irish_data[train_num:]

print("train dataset is :", len(train_list))
print("eval dataset is :", len(eval_list))


woptions = whisper.DecodingOptions(language="English", without_timestamps=True)
wmodel = whisper.load_model("small")
wtokenizer = whisper.tokenizer.get_tokenizer(True, language="English", task=woptions.task)
...
dataset = IrishSpeechDataset(eval_list, wtokenizer, SAMPLE_RATE)
...
model = WhisperModelModule(cfg, model_name, lang, train_list, eval_list)
